...
ResultString+=`curl 'http://'$DomoticzIP':'$DomoticzPort'/json.htm?type='$Apichange'devices&rid=15' 2>/dev/null | jq -r .result[]."Temp"`
ResultString+="°C\n"

ResultString+="-CPU Usage: "
ResultString+=`curl 'http://'$DomoticzIP':'$DomoticzPort'/json.htm?type='$Apichange'devices&rid=16' 2>/dev/null | jq -r .result[]."Data"`
ResultString+="\n"

ResultString+="-Memory Usage: "
ResultString+=`curl 'http://'$DomoticzIP':'$DomoticzPort'/json.htm?type='$Apichange'devices&rid=12' 2>/dev/null | jq -r .result[]."Data"`
ResultString+="\n"

ResultString+="-SD usage: "
ResultString+=`curl 'http://'$DomoticzIP':'$DomoticzPort'/json.htm?type='$Apichange'devices&rid=13' 2>/dev/null | jq -r .result[]."Data"`
ResultString+="\n"
...
##############################################################################
InstalledFW=`curl "http://"$DomoticzIP":"$DomoticzPort"/json.htm?type='$Apichange'hardware&filter=idx=1" 2>/dev/null | jq -r '.result[] | select(.Name=="RFXCOM")| .version'`
