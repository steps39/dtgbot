...
--~ 		print("- is an existing dtgbot command.")
  ...
  if LastCommand[SendTo]["prompt"] then
    print("Returned from prompt..looking up:",LastCommand[SendTo]["button"])
...
function PopulateMenuTab(opt)
  ...
  Deviceslist = device_list("devices")
...
function MakeRoomMenus()
  ...
  repeat
    ...
    if result ~= nil then
      ...
      print_to_log('For room '..room_number..' got some devices')
      menu_submenus[room_name] = {whitelist="",showdevstatus="y",buttons={}}
      for k,record in pairs(result) do
        if type(record) == "table" then
          menu_submenus[room_name].buttons[record.Name] = {whitelist = ""}
        end
      end
    end
  until room_number > 1
